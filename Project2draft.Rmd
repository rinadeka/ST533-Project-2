...
Let's look at median age. We can check for spatial dependence using the Moran's I test and the Geary's C test.
...
We find our Moran's I statistic to be 0.4210, which is significantly different than our expectation under the assumption of normality of -0.0101 with a p-value of 1.9e-11. This result indicates positive spatial dependence, indicating that nearby counties have more similar median ages. Now, let's check this result by calculating Geary's C.

```{r, results='hide'}
geary.test(nc_counties$food, listw = Wlist, randomisation = FALSE, alternative = 'greater')
```

We find the Geary's C statistic to be 0.5928, which is significantly different than our expectation under normality of 1 with a p-value of 1.7e-9. This result also indicates positive spatial dependence. 

Overall, we can conclude that there is positive spatial dependence in median age in NC counties.

### Opportunity Youth

Let's look at opportunity youth in 2022. We can check for spatial dependence using the Moran's I test and the Geary's C test.
...
We find the Geary's C statistic to be 0.5560, which is significantly different than our expectation under normality of 1 with a p-value of 4.849e-11. This result also indicates positive spatial dependence. 
...
We decided to go with Overdose Deaths.
